import React from 'react';

interface StarRatingProps {
  value: number;         // 1–5
  onChange?: (v: number) => void; // undefined = read-only
  size?: 'sm' | 'md' | 'lg';
}

const SIZES = { sm: 'h-3.5 w-3.5', md: 'h-5 w-5', lg: 'h-7 w-7' };

export const StarRating: React.FC<StarRatingProps> = ({ value, onChange, size = 'md' }) => {
  const [hovered, setHovered] = React.useState(0);
  const isInteractive = !!onChange;
  const display = isInteractive && hovered > 0 ? hovered : value;
  const cls = SIZES[size];

  return (
    <div
      className={`flex items-center space-x-0.5 ${isInteractive ? 'cursor-pointer select-none' : ''}`}
      onMouseLeave={() => isInteractive && setHovered(0)}
    >
      {[1, 2, 3, 4, 5].map(star => (
        <svg
          key={star}
          className={`${cls} transition-colors ${
            star <= display ? 'text-transit-gold' : 'text-line-gray'
          }`}
          fill="currentColor"
          viewBox="0 0 20 20"
          onMouseEnter={() => isInteractive && setHovered(star)}
          onClick={() => isInteractive && onChange!(star)}
        >
          <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
        </svg>
      ))}
      {!isInteractive && value > 0 && (
        <span className="text-[11px] font-mono text-slate ml-1">{value.toFixed(1)}</span>
      )}
    </div>
  );
};
