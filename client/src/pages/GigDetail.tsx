      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card variant="teal" className="text-left space-y-3">
            <div className="bg-cream border-2 border-ink rounded-xl p-5 shadow-retro-sm space-y-3">
              ...
              <h1 className="text-xl font-black font-display text-ink uppercase tracking-tight leading-tight">
                {gig.title}
              </h1>

              <div className="flex flex-wrap items-center gap-4 font-mono text-xs text-ink/70 pt-2 border-t border-ink/10">
          ...
          {/* Description */}
          <Card variant="amber" className="text-left space-y-3">
            <h2 className="text-xs font-bold font-display text-ink uppercase tracking-widest pl-1">Project Description</h2>
            <div className="bg-cream border-2 border-ink rounded-xl p-5 shadow-retro-sm">
              <p className="text-sm text-ink leading-relaxed whitespace-pre-wrap font-sans font-medium">{gig.description}</p>
            </div>
          </Card>

          {/* Skills Required */}
          {gig.skillsRequired.length > 0 && (
            <Card variant="pink" className="text-left space-y-3">
              <h2 className="text-xs font-bold font-display text-ink uppercase tracking-widest pl-1">Skills Required</h2>
              <div className="bg-cream border-2 border-ink rounded-xl p-4 shadow-retro-sm flex flex-wrap gap-2">
          ...
          {user?.role === 'freelancer' && gig.status === 'open' && (
            <Card variant="coral" className="text-left space-y-3">
              <h2 className="text-xs font-bold font-display text-ink uppercase tracking-widest pl-1">
                {proposal ? 'Your Proposal Bid' : 'Submit Proposal Bid'}
              </h2>

              <div className="bg-cream border-2 border-ink rounded-xl p-5 shadow-retro-sm space-y-4">
                {applyMsg && (
                    ...
                    {proposal.status === 'negotiating' && proposal.lastProposedBy === 'client' && (
                      <div className="border-t border-ink/20 pt-3 mt-3 space-y-3 bg-accent-teal/5 p-3 rounded-lg border-2">
                        ...
                        <div className="flex items-center space-x-2 pt-1">
                          ...
                          <Button
                            onClick={() => {
                              const amt = Number(counterInput);
                              if (amt > 0) handleProposalResponse('negotiate', amt);
                            }}
                  ...
                  <form onSubmit={handleApply} className="space-y-4 font-sans">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="space-y-1 text-left">
                        <label className="text-[10px] font-bold font-display uppercase tracking-widest text-ink pl-1">Your Bid Amount (₹) *</label>
                        <Input
                          type="number"
                          required
                          min="1"
                      ...
                      <div className="space-y-1 text-left">
                        <label className="text-[10px] font-bold font-display uppercase tracking-widest text-ink pl-1">Delivery Time (Days) *</label>
                        <Input
                          type="number"
                          required
                          min="1"
                    ...
                    <div className="space-y-1.5 text-left">
                      <div className="flex items-center justify-between">
                        <label className="text-[10px] font-bold font-display uppercase tracking-widest text-ink pl-1">
                        ...
                        <button
                          ...
                          disabled={draftingAI}
                          className="text-[10px] font-mono font-bold text-accent-teal hover:underline flex items-center space-x-1 cursor-pointer"
                        >
                          <span>✨ {draftingAI ? 'Drafting AI...' : 'Draft with AI'}</span>
                        </button>
                      </div>
                      <textarea
                        required
                        rows={4}
                        placeholder="Explain why you're a great fit, relevant experience, timeline..."
                        value={message}
                        onChange={e => setMessage(e.target.value)}
                        className="w-full px-4 py-2.5 bg-cream border-2 border-ink rounded-lg text-ink text-sm resize-none focus:outline-none focus:bg-accent-amber/10 focus:border-accent-amber"
                      />
                    </div>
                    ...
                    >
                      ...
                      <span>{applying ? 'Submitting Bid...' : 'Submit Proposal Bid'}</span>
                    </Button>
                  </form>
                )}
              </div>
          ...
          {gig.status === 'in_progress' && (
            <div className="space-y-6">
              ...
              {/* 1. Milestone Checklist Section */}
              <Card variant="teal" className="text-left space-y-4">
                <div className="flex items-center justify-between border-b-2 border-ink pb-3">
                  <div className="flex items-center space-x-2">
                    <PenLine className="h-4 w-4 text-accent-teal" />
                    <h2 className="text-xs font-bold font-display text-ink uppercase tracking-widest">Milestones & Deliverables</h2>
                  </div>
                  <Badge variant="teal" className="shadow-none font-mono text-[9px]">
                    {gig.milestones?.filter((m: any) => m.status === 'completed').length || 0} / {gig.milestones?.length || 0} COMPLETED
                  </Badge>
                </div>
                ...
                <div className="bg-cream border-2 border-ink rounded-xl p-5 shadow-retro-sm space-y-4">
                  {(!gig.milestones || gig.milestones.length === 0) ? (
                    <div className="text-left space-y-3">
                      <p className="text-xs text-ink/70 font-sans">No deliverables created yet. Break down the work into verifiable milestones.</p>
                      
                      {user?._id === gig.clientId._id && (
                        <div className="space-y-3 pt-2">
                          {newMilestones.map((m, idx) => (
                            <div key={idx} className="border-2 border-ink/20 p-3 bg-cream rounded-lg space-y-2">
                              <div className="flex items-center space-x-2">
                                <Input
                                  ...
                                  className="py-1.5 text-xs flex-grow"
                                />
                                <Input
                                  type="date"
                                  value={m.dueDate}
                      ...
                      {gig.milestones.map((m: any) => (
                        <div key={m._id} className="border-2 border-ink p-4 bg-cream rounded-xl flex flex-col sm:flex-row sm:items-start justify-between gap-4 shadow-retro-sm">
                          <div className="space-y-1 text-left">
                            ...
                            <p className="text-ink/70 text-xs mt-1 font-sans font-medium">{m.description}</p>
                            {m.dueDate && (
              ...
              <Card variant="amber" className="text-left space-y-3">
                <h2 className="text-xs font-bold font-display text-ink uppercase tracking-widest pl-1">Project Logs</h2>
                
                <div className="bg-cream border-2 border-ink rounded-xl p-4 shadow-retro-sm space-y-3">
                  {/* Scrolling Logs */}
                  <div className="border-2 border-ink p-3 bg-cream/50 max-h-48 overflow-y-auto space-y-2 font-mono text-[10px] text-ink rounded-lg text-left shadow-retro-sm">
        ...
        <div className="space-y-6">
          ...
          {/* Budget card */}
          <Card variant="amber" className="text-left space-y-3">
            <h3 className="text-xs font-bold font-display text-ink uppercase tracking-widest pl-1">Budget</h3>
            <div className="bg-cream border-2 border-ink rounded-xl p-4 shadow-retro-sm flex items-baseline space-x-1 font-mono">
          ...
          <Card variant="teal" className="text-left space-y-3">
            ...
            <div className="bg-cream border-2 border-ink rounded-xl p-4 shadow-retro-sm space-y-4">
              ...
              <div className="space-y-2 text-xs font-mono text-ink/60 border-t border-ink/10 pt-3">
                ...
                <div className="flex items-center justify-between">
                  <span className="flex items-center space-x-1"><MapPin className="h-3.5 w-3.5 text-accent-teal" /><span>Location</span></span>
          ...
          {(gig.status === 'in_progress' || gig.status === 'completed') && (
            (() => {
              ...
              return (
                <Card variant="pink" className="space-y-3 text-left">
